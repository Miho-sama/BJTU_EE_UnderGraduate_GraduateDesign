...
//gamma
	...
	LCD_WriteReg(0x0003, 0x1018);       //1030	设置增长方向  P42：Entry Mode (R003h)
	...
	LCD_WriteReg(0x0210, 0x0000);	// 水平起始地址
	LCD_WriteReg(0x0211, 239);		// 水平结束坐标
	LCD_WriteReg(0x0212, 0x0000);	// 垂直起始地址 
	LCD_WriteReg(0x0213, 399);   	// 垂直结束地址    //432=01AF,400=018F
	...
	for(i=0;i<400;i++)
	 {
	  for (j=0;j<240;j++)
		 ...
		 temp+=*p;
		 LCD->LCD_RAM=temp;
		 //R61509V_WR_DATA((*p)+(*(p+1))<<8);  //这样算越界了！！！
		p=p+2;
	    }
...
static void LCD_FSMCConfig(void)
{
  ...
  FSMC_NORSRAMTimingInitStructure.FSMC_AccessMode = FSMC_AccessMode_A;//FSMC_AccessMode_B
...
void dis_a_num(uint16_t Xpos,uint16_t Ypos, uint8_t c)
{
	

	LCD_DrawChar(Xpos, Ypos, &Ascii16[(c+48 )* 16], 8);	/* 16 表示1个字符字模的字节数 */	

}

void dis_char_num(uint16_t Xpos,uint16_t Ypos, uint8_t c)
{
  uint8_t t[4]={0};	
  uint8_t i=0;
  uint8_t j=0;
  ...
  LCD_DisplayString(Xpos,Ypos,"   ");
  for(j=0;j<i;j++)
  {
	dis_a_num(Xpos+8*j,Ypos,t[i-j-1]) ; 	
  }

}

void dis_int_num(uint16_t Xpos,uint16_t Ypos, uint16_t c)
{
  uint8_t t[6]={0};	
  uint8_t i=0;
  uint8_t j=0;
...
void Test_FSMC_WR(uint16_t d)
{
	LCD->LCD_RAM =d; // ((uint32_t)(0x60000000 | 0x0C000000))+1 
}
uint16_t Test_FSMC_RD(void)
 {
	return (LCD->LCD_RAM);	
 }
