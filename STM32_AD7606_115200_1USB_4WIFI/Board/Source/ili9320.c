...
void ili9320_Initializtion(void)
{
  ...
  ******************************/
  uint16_t i;
  LCD_X_Init();
  Delay_ms(100); /* Delay_ms 50 ms */			//start internal osc
...
****************************************************************************/
__inline void ili9320_SetCursor(uint16_t x,uint16_t y)
{
...
****************************************************************************/
__inline void ili9320_SetWindows(uint16_t StartX,uint16_t StartY,uint16_t EndX,uint16_t EndY)
{
...
/****************************************************************************
* 名    称：uint16_t ili9320_GetPoint(uint16_t x,uint16_t y)
* 功    能：获取指定座标的颜色值
* 入口参数：x      行座标
*           y      列座标
* 出口参数：当前座标颜色值
...
/****************************************************************************
* 名    称：void ili9320_BackLight(uint8_t status)
* 功    能：开、关液晶背光
...
****************************************************************************/
void ili9320_DisplayStringLine(uint8_t Line, uint8_t *ptr, uint16_t charColor, uint16_t bkColor)
{
  uint32_t i = 0;
...
void ili9320_Display_NUM(uint16_t x, uint8_t Line,  uint32_t NUM, uint16_t charColor, uint16_t bkColor)
{
	uint16_t i=0;
	uint16_t j=0;
	uint8_t shuzi[11]={0}; //0xFFFFFFFF占十进制的 10位
    ...
    while(NUM/10)
	{
		shuzi[i++]=NUM%10+0x30;		  //从低到高依次获取有效的十进制位，并且转化为相应的assii，如 0 的assii 是0x30 
		NUM=NUM/10;
			
	}
	shuzi[i++]=NUM%10+0x30;			 //获取最高位


		for(j=0;j<i;j++)
		{
			ili9320_PutChar(x, Line, shuzi[i-j-1], charColor, bkColor);
			x+=8;	
		}
	    for(;j<10;j++)
		{
			ili9320_PutChar(x, Line, ' ', charColor, bkColor);	
			x+=8;
		}
